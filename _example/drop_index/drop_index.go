...
func main() {
	...
	flag.StringVar(&outFilePath, "o", "./_example/sql/drop_index.sql", "set ddl output file path")
	flag.StringVar(&outFilePath, "outfile", "./_example/sql/drop_index.sql", "set ddl output file path")
	flag.Parse()
	...
	for i := range ebs {
		schemas, err := cli.GenerateDropIndexes(ebs[i])
		if err != nil {
			log.Println(err.Error())
			return
		}
